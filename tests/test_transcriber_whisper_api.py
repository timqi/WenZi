...
class TestInitialize:
    def test_initialize_creates_openai_client(self):
        ...
        with patch("voicetext.transcriber_whisper_api.OpenAI", return_value=mock_client) as mock_cls:
            t.initialize()
    ...
    def test_initialize_sets_initialized_flag(self):
        t = _make_transcriber()
        with patch("voicetext.transcriber_whisper_api.OpenAI"):
            t.initialize()
        assert t._initialized is True
...
class TestVerifyProvider:
    def test_returns_none_on_success(self):
        ...
        with patch("voicetext.transcriber_whisper_api.OpenAI", return_value=mock_client):
    ...
    def test_returns_error_string_on_failure(self):
        with patch(
            "voicetext.transcriber_whisper_api.OpenAI",
