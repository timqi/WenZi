...
class TestInitialize:
    ...
    def test_initialize_calls_loaders_and_sets_flag(self):
        ...
        with patch.object(t, "_load_asr", return_value=True) as mock_asr, \
            ...
            with patch("voicetext.transcriber_funasr.threading.Thread") as mock_thread:
...
class TestCleanup:
    ...
    def test_cleanup_calls_gc_collect(self):
        t = FunASRTranscriber()
        t._initialized = True
        with patch("voicetext.transcriber_funasr.gc.collect") as mock_gc:
            t.cleanup()
...
class TestTranscribe:
    ...
    def test_transcribe_cleans_up_temp_file(self):
        ...
        with pytest.raises(RuntimeError, match="asr error"):
            with patch("voicetext.transcriber_funasr.os.unlink") as mock_unlink:
                t.transcribe(_make_wav())
    ...
    def test_transcribe_temp_file_oserror_suppressed(self):
        ...
        with patch("voicetext.transcriber_funasr.os.unlink", side_effect=OSError("no file")):
            result = t.transcribe(_make_wav())
        assert result == "ok"
...
class TestGetModelDir:
    ...
    def test_downloads_when_cache_missing(self, tmp_path):
        ...
        t = FunASRTranscriber()
        with patch("pathlib.Path.home", return_value=tmp_path), \
             patch("voicetext.transcriber_funasr.FunASRTranscriber._get_model_dir",
            ...
            with patch.dict("sys.modules", {
                ...
                with patch("voicetext.transcriber_funasr.FunASRTranscriber._get_model_dir",
...
class TestLoaders:
    ...
    def test_load_punc_restorer_success(self):
        ...
        # Build a fake voicetext.punctuation module
        ...
        import sys
        real_module = sys.modules.get("voicetext.punctuation")
        sys.modules["voicetext.punctuation"] = mock_punc_module
        try:
            result = t._load_punc_restorer()
        finally:
            if real_module is None:
                sys.modules.pop("voicetext.punctuation", None)
            else:
                sys.modules["voicetext.punctuation"] = real_module
    ...
    def test_load_punc_restorer_failure(self):
        t = FunASRTranscriber()
        with patch.dict("sys.modules", {"voicetext.punctuation": None}):
            result = t._load_punc_restorer()
        assert result is False
